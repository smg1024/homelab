...
This roadmap tracks completed work and planned changes. Checked items link to
what shipped. Unchecked items are roughly ordered by priority within each
horizon.
...
- [x] Run [jamye-plz](services/applications.md) on alfheim to validate the
      cloud-host pattern
...
- [x] Replace the Grafana/Prometheus/Loki/Alloy stack with
      Beszel + VictoriaLogs: lighter on yggdrasil's 4 GB, simpler UIs,
      and working alert delivery ([details](services/monitoring.md))
- [ ] Per-page edit buttons on this site (`content.action.edit` +
      Forgejo `edit_uri`)
- [ ] Review whether any tailnet-only surface needs Cloudflare Access
      as a second factor
