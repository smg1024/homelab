...
- 콘텐츠와 구조를 그대로 이전했으며 2개 언어 분리 빌드(`/` + `/ko/`)
  구조도 동일하게 동작합니다.
- 빌드는 nixpkgs의 `pkgs.zensical`만 사용하는 순수 Nix 구성을 유지하며 추가
  툴체인이 없습니다.
- Zensical은 0.0.x라는 위험을 감수합니다. 산출물이 정적 HTML이라 빌드 실패가
  호스트에 영향을 주지 않고 flake 핀이 검증된 버전을 고정합니다. 문서화된
  대안인 mkdocs-material로 되돌릴 수도 있습니다.
- 일부 설정은 문서와 구현이 다릅니다. 예를 들어 generator 표기 토글은
  `project`가 아니라 `extra` 아래에 있습니다.
