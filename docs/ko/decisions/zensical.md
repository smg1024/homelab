...
| 선택지 | 평가 |
| --- | --- |
| mkdocs-material 유지 (핀 고정) | flake 핀 덕에 당장은 동작하지만, 유지보수 끝난 토대 위에 쌓는 것 |
| Astro Starlight / MDX 계열 | 룩은 최고지만 npm 툴체인 → 업데이트마다 `buildNpmPackage` 해시 갱신 |
| mdBook | nixpkgs에 있고 매우 안정적이지만 i18n이 없고 룩이 소박함 |
| **Zensical** | Material 팀의 후속작; MkDocs 형식 설정을 읽음; nixpkgs 최상위 패키지 |

## 결정

**Zensical**로 이전하고, 설정은 정식 형식인 `zensical.toml`을 사용한다
...
- 수용한 리스크: Zensical은 0.0.x. 완화책 — 산출물이 정적 HTML이라 실패가
  호스트에 영향 줄 수 없고, flake 핀이 검증된 버전을 고정하며, 문서화된
  대안은 여전히 빌드 가능한 mkdocs-material로의 회귀.
