...
접근 주체와 허용 범위를 공개 인터넷, tailnet, localhost의 세 가지 경계로
나눕니다.
...
| **tailnet** | Tailscale tailnet에 속한 기기 | 위 공개 서비스 + `beszel`·`logs` 라우트 + Tailscale ACL에 따른 호스트/포트 직접 접근 (신뢰된 `tailscale0` 인터페이스로 Beszel `:8090`, VictoriaLogs `:9428`, vlagent `:9429` 등 도달 가능) |
...
- **ipTIME 공유기 상태**. DHCP 고정 할당, NAT, 포트 포워딩 규칙은 공유기에
  있습니다.
