...
파일입니다. [Zensical](https://zensical.org/)은 Material for MkDocs 팀이 만든
정적 사이트 생성기입니다. 빌드한 결과는 yggdrasil의 Caddy가 `file_server`로 Nix
스토어에서 직접 서빙합니다. 공개 트래픽은 Cloudflare Tunnel을 거쳐 같은
Caddy로 들어옵니다. 설정은 Zensical 공식 형식인 `zensical.toml`을
사용합니다.

기본 언어는 영어이며 한국어 번역본이 `/ko/` 아래 별도 서브사이트로
빌드됩니다. 헤더의 언어 선택기로 언어를 전환합니다.
...
- 서빙: `services/ingress.nix`의 `docs.ridewithmin.com` vhost가 yggdrasil의
  Caddy `file_server`로 빌드된 패키지를 직접 서빙, Cloudflare Tunnel로
  공개

## 문서 수정 워크플로

1. 먼저 `docs/en/`의 영어 페이지를 수정하고 `docs/ko/`에 같은 변경을
   반영합니다. 새 페이지는 양쪽 언어에서 다음 두 항목도 수정합니다.
