...
디스크 장애 시 유지되는 항목과 사라지는 데이터, 복구 절차를 설명합니다.
...
모든 호스트의 시스템 구성은 저장소에서 재현합니다.
[부트스트랩 런북](bootstrap-host.md)으로 머신을 처음부터 재구축하므로
별도 백업이 필요하지 않습니다.

반면 다음 데이터는 호스트에만 존재합니다.
