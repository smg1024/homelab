...
배포합니다. 수동 [`just` 워크플로](deploy.md)는 비상 수단(break-glass)으로
남습니다 — 파이프라인은 같은 `nixos-rebuild`를 무인으로 실행할 뿐입니다.

이렇게 나눈 것은 의도적입니다.

- **CI**는 모든 호스트가 *빌드되는지* 검증합니다 — 어떤 호스트도 건드리지 않는
  일회용 러너에서.
- **CD**는 병합된 변경을 배포합니다 — 테일넷에 합류해 손으로 실행할 때와 똑같은
  `nixos-rebuild switch`를 실행합니다.
...
PR / 푸시 ──▶ CI: 각 호스트의 시스템 클로저 빌드   (빌드만, 호스트는 건드리지 않음)
                 └─ 필수 체크가 병합을 막는 게이트
main에 병합 ──▶ CD: 테일넷 합류 → 모든 호스트에서 nixos-rebuild switch
```

## CI — 빌드 체크

`.github/workflows/ci.yml`, `main`으로 향하는 풀 리퀘스트에서 트리거됩니다.

- 호스트마다 하나의 잡이
  `nixosConfigurations.<host>.config.system.build.toplevel`을 빌드합니다.
- **빌드만 합니다** — 활성화도, 테일넷 접근도, 비밀도 없습니다. 평가나 컴파일에
...
빌드합니다** — 수동 배포와 똑같습니다. 러너는 플레이크를 평가하고
...
비밀번호 없는 `sudo`(`security.sudo.wheelNeedsPassword = false`)를 가지므로
호스트 쪽 추가 설정은 필요 없습니다.

!!! warning "부트스트랩 순서"
    러너는 호스트가 배포 키를 이미 신뢰할 때에만 로그인할 수 있습니다. 그 키의
    첫 배포는 수동입니다: 각 호스트를 [`just switch`](deploy.md)한 *다음* CD에
    넘깁니다. 그 이후로는 병합이 알아서 배포합니다.
...
- **자동 롤백이 없습니다.** 병합 `switch`에는 매직 롤백이 없고, 초록색 CI는
  ...
  롤백합니다.
- **비상 수단은 수동으로 유지합니다.** 파이프라인이 막히거나 호스트에 긴급
  수정이 필요하면 [`just switch`](deploy.md)로 직접 배포하고
  `sudo nixos-rebuild switch --rollback`으로 되돌립니다.
