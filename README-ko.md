...
이 repo는 NixOS 머신으로 구성된 homelab의 source of truth다. 호스트
...
`flake.nix`는 NixOS 26.05를 pinning하고, 세 NixOS configuration을 노출한다.
...
- Cloudflare Tunnel을 통해 이 문서 사이트 공개 제공
...
`alfheim`은 실험용 Oracle Cloud Infrastructure ARM VM이다.

주요 역할:

- OCI 위에서 NixOS 운영 검증
- 영구 서비스를 올리기 전 cloud-hosted homelab 패턴 테스트
- 공통 운영 baseline을 가진 작은 원격 노드 제공

현재는 공통 모듈만 로드하고, application 전용 서비스는 올리지 않는다.
SSH는 의도적으로 tailnet을 통해서만 노출하며, public OCI 주소에서는 SSH를
받지 않는다.
...
모든 호스트는 단일 디스크의 단순한 GPT 레이아웃을 사용한다.
...
docs.ridewithmin.com   -> services/docs-site.nix의 정적 파일
...
`docs.ridewithmin.com`은 빌드된 문서 사이트를 `yggdrasil`에서 정적 파일로
서빙하며 Cloudflare Tunnel을 거친다.
...
Prometheus와 Grafana는 `yggdrasil`에서 실행된다. Prometheus는 scrape config에
등록된 service host의 node-level metric을 수집하고, Grafana는 tailnet으로 제한된
Caddy route를 통해 운영자 dashboard를 제공한다. Uptime Kuma는 계속 public
endpoint 확인과 public status page를 담당한다.
...
현재 구성에서 NixOS firewall은 모든 호스트에서 활성화되어 있다. 홈 서비스
호스트는 SSH `22`를 직접 허용하지만, `alfheim`은 trusted `tailscale0`
interface를 통해서만 SSH를 허용한다. `3000`, `3001`, `8082`, `8222`, `9090`,
`9100` 같은 application 및 monitoring port는 일반 public firewall port로 열지
않는다.
...
모든 호스트는 `tailscale0` interface를 trusted interface로 둔다. 따라서
